classdef Tree < hgsetget
    ...
    %
    % Syntax:
    %   nObj = UserInterface.uiextras.jTree.Tree
    %   nObj = UserInterface.uiextras.jTree.Tree('Property','Value',...)
    %
    % Tree Properties:
    %
    ...
    %   f = figure;
    %   t = UserInterface.uiextras.jTree.Tree('Parent',f,...
    %       'SelectionChangeFcn','disp(''SelectionChangeFcn triggered'')');
    %
    %   %% Create tree nodes
    %   Node1 = UserInterface.uiextras.jTree.TreeNode('Name','Node_1','Parent',t.Root);
    %   Node1_1 = UserInterface.uiextras.jTree.TreeNode('Name','Node_1_1','Parent',Node1);
    %   Node1_2 = UserInterface.uiextras.jTree.TreeNode('Name','Node_1_2','Parent',Node1);
    %   Node2 = UserInterface.uiextras.jTree.TreeNode('Name','Node_2','Parent',t.Root);
    %
    %   %% Set an icon
    ...
    %   t.Enable = 'on';
    %
    % See also: UserInterface.uiextras.jTree.CheckboxTree, UserInterface.uiextras.jTree.TreeNode,
    %           UserInterface.uiextras.jTree.CheckboxTreeNode
    ...
    properties (SetAccess={?UserInterface.uiextras.jTree.Tree, ?UserInterface.uiextras.jTree.TreeNode},...
            GetAccess={?UserInterface.uiextras.jTree.Tree, ?UserInterface.uiextras.jTree.TreeNode})
    ...
    methods
        function tObj = Tree(varargin)
            ...
            %
            % Syntax:
            %           tObj = UserInterface.uiextras.jTree.Tree('p1',v1,...)
            %
            % Inputs:
            %           Property-value pairs
            %
            % Outputs:
            %           tObj - UserInterface.uiextras.jTree.Tree object
            %
            % Examples:
            %           hFig = figure;
...
%             UserInterface.uiextras.jTree.loadJavaCustomizations();
    ...
    methods
        ...
        function buf = saveExpansionState(tObj)
            tObj.jTree.setRootVisible(true);
            row = 0;
            rowPath = tObj.jTree.getPathForRow(row);
            %rowPath = obj.TreeModel.getRoot.getPath;
            buf = [];
            for i = 0:tObj.jTree.getRowCount()-1
                path = tObj.jTree.getPathForRow(i);
                if (i==row || isDescendantOf(path, rowPath))
                    if(tObj.jTree.isExpanded(path))
                        buf(end+1) = i-row; %#ok<AGROW>
                    end
                end
            end
            tObj.jTree.setRootVisible(false);

            
        end % saveExpansionState 
        
        function s = saveTreeState(tObj)
            warnMsg  = warning('off','MATLAB:structOnObject'); 
            
            s = saveNodesAsStructs(tObj.jModel.getRoot);
            
            warning(warnMsg);
            
        end
        
        function restoreExpansionState(tObj,expData)
            tObj.jTree.setRootVisible(true);
            for i =1:length(expData)
                tObj.jTree.expandRow(expData(i));          
            end
            tObj.jTree.setRootVisible(false);
    ...
    methods (Access = protected)
        ...
        function createTree(tObj)
            ...
            tObj.Root = UserInterface.uiextras.jTree.TreeNode('Name','Root','Tree',tObj);
        ...
        function nObj = getNodeFromMouseEvent(tObj,e)
            ...
            if isempty(treePath)
                nObj  = UserInterface.uiextras.jTree.TreeNode.empty(0,1);
            else
        ...
        function onMouseClick(tObj,e)
            ...
            if callbacksEnabled(tObj)
                ...
                if ~isempty(nObj)
                    ...
                    hgfeval(tObj.MouseClickedCallback,tObj,e,nObj);
                end    
...
%             end %if callbacksEnabled(tObj)
        ...
        function onNodeSelection(tObj,e)
            ...
            if callbacksEnabled(tObj) && ~isempty(tObj.SelectionChangeFcn)
                ...
                % selection
                p = e.getPaths;
                AddedNodes = UserInterface.uiextras.jTree.TreeNode.empty(0,1);
                RemovedNodes = UserInterface.uiextras.jTree.TreeNode.empty(0,1);
                for idx = 1:numel(p)
        ...
        function onNodeEdit(tObj,e)
            ...
            if callbacksEnabled(tObj) && ~isempty(tObj.NodeEditedCallback)
                ...
                c = e.getChildren;
                EditedNode = UserInterface.uiextras.jTree.TreeNode.empty(0,1);
                for idx = 1:numel(c)
        ...
        function onNodeDND(tObj,e)
            ...
            if callbacksEnabled(tObj)
                ...
                SourceNode = UserInterface.uiextras.jTree.TreeNode.empty(0,1);
    ...
    methods
        ...
        function set.FontAngle(tObj, value)
            ...
            switch value
                ...
                case 'oblique'
                    error('UserInterface.uiextras:Tree:InvalidArgument', ...
        ...
        function value = get.SelectedNodes(tObj)
            value = UserInterface.uiextras.jTree.TreeNode.empty(0,1);
        ...
        function set.SelectedNodes(tObj,value)
            ...
            elseif isa(value,'UserInterface.uiextras.jTree.TreeNode')
                if isscalar(value)
                    tObj.CBEnabled = false;
...
function nodeStruct = saveNodesAsStructs(node)
    nodeStruct = node2struct(node);
    for i = 1:node.getChildCount 
        nodeStruct = [nodeStruct,saveNodesAsStructs(node.getChildAt(i-1))]; %#ok<AGROW>
    end
...
function outStruct = node2struct(node)
    ...
    if isempty(node.getIcon)
        icon = [];
    else
        icon = char(node.getIcon.toString);
    end
    outStruct = struct('Name',mNode.Name,...
                        'TooltipString',mNode.TooltipString,...
                        'Value',mNode.Value,...
                        'UserData',mNode.UserData,...
                        'UIContextMenu',mNode.UIContextMenu,...
                        'TreePath',char(node.getTreePath.toString),...
                        'Icon',icon,...
                        'Depth',node.getDepth);     
...
function y = isDescendantOf(path1,path2)
