classdef TreeNode < hgsetget & matlab.mixin.Heterogeneous
    ...
    %   UserInterface.uiextras.jTree.Tree control.
    %
    % Syntax:
    %   nObj = UserInterface.uiextras.jTree.TreeNode
    %   nObj = UserInterface.uiextras.jTree.TreeNode('Property','Value',...)
    %
    % TreeNode Properties:
    %
    ...
    %
    % See also: UserInterface.uiextras.jTree.Tree, UserInterface.uiextras.jTree.CheckboxTree,
    ...
    properties
        Parent = UserInterface.uiextras.jTree.TreeNode.empty(0,1) %Parent tree node
        UIContextMenu %context menu to show when clicking on this node
    end
    
    properties (SetAccess=protected, GetAccess=public)
        Children = UserInterface.uiextras.jTree.TreeNode.empty(0,1) %Child tree nodes
        Tree = UserInterface.uiextras.jTree.Tree.empty(0,1) %Tree on which this node is attached
    end
...
%             UserInterface.uiextras.jTree.loadJavaCustomizations();
    ...
    methods
        ...
        function tf = isAncestor(nObj1,nObj2)
            ...
            %           tf - logical result
            %
            validateattributes(nObj1,{'UserInterface.uiextras.jTree.TreeNode'},{'vector'})
            validateattributes(nObj2,{'UserInterface.uiextras.jTree.TreeNode'},{'vector'})
        ...
        function setIcon(nObj,icon)
            ...
            %
            % Examples:
            %   t = UserInterface.uiextras.jTree.Tree;
            %   n = UserInterface.uiextras.jTree.TreeNode('Name','Node1','Parent',t);
    ...
    methods (Access = protected)
        ...
        function newParent = updateParent(nObj,newParent)
            ...
            if isempty(newParent)
                ...
                % [] was passed in
                newParent = UserInterface.uiextras.jTree.TreeNode.empty(0,1);
