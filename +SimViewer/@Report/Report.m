...
%             newTable1.Rows.Alignment = 0;            
        ...
        function addOperCondTable( obj , operCond , header )
            ...
            % Determine units and numeric flags
            ...
            nr_cols = numel(selFields) + 1; % extra color column

            % Add Table
            newTable1 = obj.ActX_word.ActiveDocument.Tables.Add(range,nr_rows,nr_cols,1,1);

            % ---- Header rows ----
            % Second row: variable names with units (exclude color column)
            ...
            c = 1;
            while c <= numel(selFields)
                startIdx = c;
                curType = cats{c};
                while c < numel(selFields) && strcmp(cats{c+1},curType)
                    c = c + 1;
                end
                if startIdx ~= c
            ...
            for i = 1:length(operCond)
                if mod(i,2) == 0
                    for cc = 1:numel(selFields)
                        newTable1.Cell(i+2,cc).Shading.BackgroundPatternColor = stripeColor;
                    end
                end
                ...
                % Color column
                newTable1.Cell(i + 2,numel(selFields)+1).Shading.BackgroundPatternColor = Utilities.DHX(operCond(i).Color);
            end
            ...
            try
                newTable1.Columns.Item(nr_cols).SetWidth(15,0);
            catch
            end
