classdef GainCollectionNew < matlab.mixin.Copyable & hgsetget
    
    %% Public properties - Data Storage
    properties  
        DesignOperatingCondition lacm.OperatingConditionCtrl = lacm.OperatingConditionCtrl.empty
...
%     end % Public properties
    ...
    properties (Hidden = true)
        Selected
        Color
        GainComparisonArray string
        SynthesisComparisonArray string
        RequirementComparisonArray string
...
%         end % ReqTableData - Get
    ...
    methods 
        ...
        function addGain ( obj, gain, idx )
            gainIdx = find(strcmp(gain.Name, obj.GainComparisonArray));
            if isempty(gainIdx);
                gainIdx = length(obj.Gain) + 1;
                obj.GainComparisonArray(gainIdx) = gain.Name;
                obj.Gain(gainIdx) = ScatteredGain.Gain;
                obj.Gain(gainIdx).Name = gain.Name;
                obj.Gain(gainIdx).Expression = gain.Expression;
                obj.Gain(gainIdx).Value = single(nan(length(obj.Gain(1).Value), 1));
            end
            obj.Gain(gainIdx).Value(idx,:) = single(gain.Value);
        end

        function addSynthesisDesignParameter ( obj, parameter, idx )
            parameterIdx = find(strcmp(parameter.Name, obj.SynthesisComparisonArray));
%             valueLength = size(parameter.Value,2);
            valueLength = length(parameter.Value);
            if isempty(parameterIdx)
                parameterIdx = length(obj.SynthesisDesignParameter) + 1;
                obj.SynthesisComparisonArray(parameterIdx) = parameter.Name;
                obj.SynthesisDesignParameter(parameterIdx) = ScatteredGain.Parameter;
                obj.SynthesisDesignParameter(parameterIdx).Name = parameter.Name;
                obj.SynthesisDesignParameter(parameterIdx).Value = nan(length(obj.SynthesisDesignParameter(1).Value), valueLength, 'single');
                obj.SynthesisDesignParameter(parameterIdx).ValueString = strings(length(obj.SynthesisDesignParameter(1).ValueString), valueLength);
                obj.SynthesisDesignParameter(parameterIdx).ValueString(:) = "NaN";
            end
            obj.SynthesisDesignParameter(parameterIdx).Value(idx,:) = single(parameter.Value);
            obj.SynthesisDesignParameter(parameterIdx).ValueString(idx,:) = parameter.ValueString;
        end

        function addRequirementDesignParameter ( obj, parameter, idx )
            parameterIdx = find(strcmp(parameter.Name, obj.RequirementComparisonArray));
            valueLength = length(parameter.Value);
            if isempty(parameterIdx)
                parameterIdx = length(obj.RequirementComparisonArray) + 1;
                obj.RequirementComparisonArray(parameterIdx) = parameter.Name;
                obj.RequirementDesignParameter(parameterIdx) = ScatteredGain.Parameter;
                obj.RequirementDesignParameter(parameterIdx).Name = parameter.Name;
                obj.RequirementDesignParameter(parameterIdx).Value = nan(length(obj.RequirementDesignParameter(1).Value), valueLength, 'single');
                obj.RequirementDesignParameter(parameterIdx).ValueString = strings(length(obj.RequirementDesignParameter(1).ValueString), valueLength);
                obj.RequirementDesignParameter(parameterIdx).ValueString(:) = "NaN";
            end
            obj.RequirementDesignParameter(parameterIdx).Value(idx,:) = single(parameter.Value);
            obj.RequirementDesignParameter(parameterIdx).ValueString(idx,:) = parameter.ValueString;
        end
        ...
        function file = write2File( obj, ScatteredGainObjName)
            ...
            %nCond = length(obj);
            if obj.NumGainCols > 0
                fprintf(fid,'%s%s\n',' NUMBER OF DESIGN CONDITIONS',[' : ' num2str(obj.NumGainCols)]);
                fprintf(fid,'\n');
                fprintf(fid,'%s\n\n','% ---------------------------- DESIGN DATA ---------------------------------------------------');
                
                DateVec(obj.NumGainCols) = datetime;
                for i=1:obj.NumGainCols
                    DateVec(i) = datetime(obj.Date(i));
                end
                [~,isort] = sort(DateVec);
                ...
                for i=isort
                    ...
                    var{1} = 'Date';
                    val{1} = char(obj.Date(i));
                    
                    var{2} = obj.OperCondFilterSettings(i).Parameter1_FC1_Str;
                    var{3} = obj.OperCondFilterSettings(i).Parameter2_FC2_Str;
                    var{4} = obj.OperCondFilterSettings(i).Parameter3_IOC_Str;
                    var{5} = obj.OperCondFilterSettings(i).Parameter4_WC_Str;
                    
                    val{2} = obj.OperCondFilterSettings(i).Parameter1_FC1_Value;
                    val{3} = obj.OperCondFilterSettings(i).Parameter2_FC2_Value;
                    val{4} = obj.OperCondFilterSettings(i).Parameter3_IOC_Value;
                    val{5} = obj.OperCondFilterSettings(i).Parameter4_WC_Value;
                    
                    
                    for j=1:length(obj.Gain)
                        var{end+1}=obj.Gain(j).Name;
                        val{end+1}=num2str(obj.Gain(j).Value(i));
                    end
                    ...
                    for j=1:length(uniqueParams)
                        if isscalar(uniqueParams(j).Value(i))
                            var{end+1}=uniqueParams(j).Name;
                            val{end+1}=num2str(uniqueParams(j).Value(i));
                        end
                    end
