classdef CheckboxTree < UserInterface.uiextras.jTree.Tree
    % CheckboxTree - Class definition for CheckboxTree
    %   The CheckboxTree object places a checkbox tree control within a
    %   figure or container.
    %
    % Syntax:
    %           tObj = UserInterface.uiextras.jTree.CheckboxTree
    %           tObj = UserInterface.uiextras.jTree.CheckboxTree('Property','Value',...)
    %
    %   The CheckboxTree contains all properties and methods of the
    %   <a href="matlab:doc UserInterface.uiextras.jTree.Tree">UserInterface.uiextras.jTree.Tree</a>, plus the following:
    %
    ...
    %
    %   %% Create tree nodes
    %   Node1 = UserInterface.uiextras.jTree.CheckboxTreeNode('Name','Node_1','Parent',t.Root);
    %   Node1_1 = UserInterface.uiextras.jTree.CheckboxTreeNode('Name','Node_1_1','Parent',Node1);
    %   Node1_2 = UserInterface.uiextras.jTree.CheckboxTreeNode('Name','Node_1_2','Parent',Node1);
    %   Node2 = UserInterface.uiextras.jTree.CheckboxTreeNode('Name','Node_2','Parent',t.Root);
    %
    %   %% Set an icon
    ...
    %   t.Enable = 'on';
    %
    % See also: UserInterface.uiextras.jTree.Tree, UserInterface.uiextras.jTree.TreeNode,
    ...
    methods
        function tObj = CheckboxTree(varargin)
            ...
            %
            % Syntax:
            %           tObj = UserInterface.uiextras.jTree.CheckboxTree('p1',v1,...)
            %
            % Inputs:
            %           Property-value pairs
            %
            % Outputs:
            %           tObj - UserInterface.uiextras.jTree.CheckboxTree object
            %
            % Examples:
            %           hFig = figure;
            ...
            tObj = tObj@UserInterface.uiextras.jTree.Tree(varargin{:});
    ...
    methods
        ...
        function s = getJavaObjects(tObj)
            s = getJavaObjects@UserInterface.uiextras.jTree.Tree(tObj);
        ...
        function s = saveTreeState(tObj)
            ...
            s = saveCBNodesAsStructs(tObj.jModel.getRoot);
    ...
    methods (Access = protected)
        ...
        function createTree(tObj)
            ...
            tObj.Root = UserInterface.uiextras.jTree.CheckboxTreeNode('Name','Root','Tree',tObj);
        ...
        function createTreeCustomizations(tObj)
            ...
            createTreeCustomizations@UserInterface.uiextras.jTree.Tree(tObj)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods (Access={?UserInterface.uiextras.jTree.Tree, ?UserInterface.uiextras.jTree.TreeNode})
        ...
        function setChecked(tObj,nObj,value)
            validateattributes(nObj,{'UserInterface.uiextras.jTree.TreeNode'},{'vector'});
...
function nodeStruct = saveCBNodesAsStructs(node)
    nodeStruct = cbNode2struct(node);
    for i = 1:node.getChildCount 
        nodeStruct = [nodeStruct,saveCBNodesAsStructs(node.getChildAt(i-1))]; %#ok<AGROW>
    end
end % saveCBNodesAsStructs

function outStruct = cbNode2struct(node)
    ...
    outStruct = struct('Name',mNode.Name,...
                        ...
                        'Checked',mNode.Checked,...
                        'PartiallyChecked',mNode.PartiallyChecked,...
                        'CheckboxEnabled',mNode.CheckboxEnabled,...
                        'CheckboxVisible',mNode.CheckboxVisible,...
                        'Value',mNode.Value,...
