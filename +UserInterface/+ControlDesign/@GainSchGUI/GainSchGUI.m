classdef GainSchGUI < matlab.mixin.Copyable & matlab.mixin.SetGet
    ...
    properties ( Access = private )  
        ...
        BreakPointsString
        IncludedInFit
        ScattGainFileObjArray ScatteredGain.GainFileNew = ScatteredGain.GainFileNew.empty
...
%         GainAdded2SelectedCollection
    ...
    methods
        ...
        % Nathan added
        function y = get.SelScattGainFile( obj )
        ...
        % Nathan added
        function y = get.SelScattGainColl( obj )
            if isempty(obj.ScattGainFileObjArray)
                y = [];
            else
                y = obj.ScattGainFileObjArray(obj.SelScattGainObjIndex).ScatteredGainCollection;
            end
        end

        % Nathan added
        function y = get.SelScattGainIdx( obj )
            if isempty(obj.SelScattGainColl)
                y = [];
            else
                y = find(obj.SelScattGainColl.Selected);
            end
        end
    ...
    methods
        ...
        function y = get.GainExpression( obj )
            ...
            if isempty(obj.SelectedGainString) || isempty(obj.SelScatteredGainList) || isempty(obj.SelectedGainIndex) || obj.SelectedGainIndex == 0
                ...
                if ~isempty(js)
                    if strcmp(ExpressionTree(1:js-1), 'Filters') % Nathan added, temporary
                        ExpressionName = strrep(ExpressionTree, 'Filters(', 'Filters(:, ');
                    else
        ...
        function y = get.XData( obj ) 
            ...
            if ~isempty(obj.IndVarExpression)%~isempty(obj.SelIndVarsList)
                evalString = [
                    '[obj.SelScattGainColl.' ...
                    obj.IndVarExpression.AccessString ...
                    ']'];
                valArray = eval(evalString);
                for i = 1:length(obj.SelScattGainIdx)
                    y(i) = valArray(obj.SelScattGainIdx(i));
                    %y(i) = eval( getString(obj.IndVarExpression,'scattGainColl(i).') );
...
%                     y(ind) = eval(['obj.FilteredScatteredGains(ind).',obj.SelIndVarsList(obj.IndVarSelectedIndex).Expression]);
        ...
        function y = get.YData( obj )
            ...
            if ~isempty(obj.GainExpression)%~isempty(obj.SelScatteredGainList)
                evalString = [
                    '[obj.SelScattGainColl.' ...
                    obj.GainExpression.AccessString ...
                    ']'];
                valArray = eval(evalString);
                for i = 1:length(obj.SelScattGainIdx)
                    y(i) = valArray(obj.SelScattGainIdx(i));
...
%                     y(ind) = eval(['obj.FilteredScatteredGains(ind).',obj.SelScatteredGainList(obj.SelectedGainIndex).Expression]);
        ...
        function y = get.GainPlotColor( obj )
            y = cell(length(obj.SelScattGainIdx), 1);
            for i = 1:length(obj.SelScattGainIdx)
                y{i} = obj.SelScattGainColl.Color(obj.SelScattGainIdx(i),:);
            end
...
%                 end
        ...
        function removeRowGainTable( obj , hobj , eventdata) 
            ...
            if any(checkedGainSchVec) %&& isempty(hobj)
...
%             obj.ScheduledGainNamesArray = names;
        ...
        function setGainSelectionComboBox( obj )
            ...
            if ~isempty(obj.SelScattGainIdx) && ~isempty(obj.SelScatteredGainList)
        ...
        function setIndVarComboBox( obj )
            ...
            if ~isempty(obj.SelScattGainIdx) && ~isempty(obj.SelIndVarsList)
    ...
    methods (Access = protected)       
        ...
        function update(obj)
            ...
            % Update the Plot
            obj.IncludedInFit = true(1,length(obj.SelScattGainIdx));
...
%                                         obj.IncludedInFit);
        ...
        function f = userUpdatesAvailableGains( obj , callingFunctionType )
            ...
            selScattGainColl = obj.SelScattGainColl;
            ...
            for i = 1:length(selScattGainColl.Gain)
                if isnumeric(selScattGainColl.Gain(i).Value(1,:)) && isscalar(selScattGainColl.Gain(i).Value(1,:))
                    CheckboxTreeNode('Name',selScattGainColl.Gain(i).Name,'Parent',gainNode,'Value',['Gain(',int2str(i),').Value']);
                end
            end
            ...
            for i = 1:length(selScattGainColl.SynthesisDesignParameter)
                if isnumeric(selScattGainColl.SynthesisDesignParameter(i).Value(1,:)) && isscalar(selScattGainColl.SynthesisDesignParameter(i).Value(1,:))
                    CheckboxTreeNode('Name',selScattGainColl.SynthesisDesignParameter(i).Name,'Parent',synDesNode,'Value',['SynthesisDesignParameter(',int2str(i),').Value']);
                end
            end
            ...
            for i = 1:length(selScattGainColl.RequirementDesignParameter)
                if isnumeric(selScattGainColl.RequirementDesignParameter(i).Value(1,:)) && isscalar(selScattGainColl.RequirementDesignParameter(i).Value(1,:))
                    CheckboxTreeNode('Name',selScattGainColl.RequirementDesignParameter(i).Name,'Parent',reqDesNode,'Value',['RequirementDesignParameter(',int2str(i),').Value']);
                end
            end
            ...
            for i = 1:size(selScattGainColl.Filters, 2)
                filterSubNode = CheckboxTreeNode('Name',selScattGainColl.Filters{1, i}.Name,'Parent',filterNode,'Value','Root');
                if ~isempty(selScattGainColl.Filters{1, i}.CenterFrequency) && isnumeric(selScattGainColl.Filters{1, i}.CenterFrequency) && isscalar(selScattGainColl.Filters{1, i}.CenterFrequency)
                    CheckboxTreeNode('Name','CenterFrequency','Parent',filterSubNode,'Value',['Filters(',int2str(i),').CenterFrequency']);
                end
                if ~isempty(selScattGainColl.Filters{1, i}.CenterAttenuation) && isnumeric(selScattGainColl.Filters{1, i}.CenterAttenuation) && isscalar(selScattGainColl.Filters{1, i}.CenterAttenuation)
                    CheckboxTreeNode('Name','CenterAttenuation','Parent',filterSubNode,'Value',['Filters(',int2str(i),').CenterAttenuation']);
                end
                if ~isempty(selScattGainColl.Filters{1, i}.SecondFrequency) && isnumeric(selScattGainColl.Filters{1, i}.SecondFrequency) && isscalar(selScattGainColl.Filters{1, i}.SecondFrequency)
                    CheckboxTreeNode('Name','SecondFrequency','Parent',filterSubNode,'Value',['Filters(',int2str(i),').SecondFrequency']);
                end
                if ~isempty(selScattGainColl.Filters{1, i}.SecondAttenuation) && isnumeric(selScattGainColl.Filters{1, i}.SecondAttenuation) && isscalar(selScattGainColl.Filters{1, i}.SecondAttenuation)
                    CheckboxTreeNode('Name','SecondAttenuation','Parent',filterSubNode,'Value',['Filters(',int2str(i),').SecondAttenuation']);
                end
                if ~isempty(selScattGainColl.Filters{1, i}.DCGain) && isnumeric(selScattGainColl.Filters{1, i}.DCGain) && isscalar(selScattGainColl.Filters{1, i}.DCGain)
                    CheckboxTreeNode('Name','DCGain','Parent',filterSubNode,'Value',['Filters(',int2str(i),').DCGain']);
                end
                if ~isempty(selScattGainColl.Filters{1, i}.HFGain) && isnumeric(selScattGainColl.Filters{1, i}.HFGain) && isscalar(selScattGainColl.Filters{1, i}.HFGain)
                    CheckboxTreeNode('Name','HFGain','Parent',filterSubNode,'Value',['Filters(',int2str(i),').HFGain']);
                end
                if ~isempty(selScattGainColl.Filters{1, i}.Frequency) && isnumeric(selScattGainColl.Filters{1, i}.Frequency) && isscalar(selScattGainColl.Filters{1, i}.Frequency)
                    CheckboxTreeNode('Name','Frequency','Parent',filterSubNode,'Value',['Filters(',int2str(i),').Frequency']);
                end
                if ~isempty(selScattGainColl.Filters{1, i}.Phase) && isnumeric(selScattGainColl.Filters{1, i}.Phase) && isscalar(selScattGainColl.Filters{1, i}.Phase)
                    CheckboxTreeNode('Name','Phase','Parent',filterSubNode,'Value',['Filters(',int2str(i),').Phase']);
                end
                if ~isempty(selScattGainColl.Filters{1, i}.Gain) && isnumeric(selScattGainColl.Filters{1, i}.Gain) && isscalar(selScattGainColl.Filters{1, i}.Gain)
                    CheckboxTreeNode('Name','Gain','Parent',filterSubNode,'Value',['Filters(',int2str(i),').Gain']);
                end
                if ~isempty(selScattGainColl.Filters{1, i}.FrequencyAtMaxPhase) && isnumeric(selScattGainColl.Filters{1, i}.FrequencyAtMaxPhase) && isscalar(selScattGainColl.Filters{1, i}.FrequencyAtMaxPhase)
                    CheckboxTreeNode('Name','FrequencyAtMaxPhase','Parent',filterSubNode,'Value',['Filters(',int2str(i),').FrequencyAtMaxPhase']);
                end
                if ~isempty(selScattGainColl.Filters{1, i}.MaxPhase) && isnumeric(selScattGainColl.Filters{1, i}.MaxPhase) && isscalar(selScattGainColl.Filters{1, i}.MaxPhase)
                    CheckboxTreeNode('Name','MaxPhase','Parent',filterSubNode,'Value',['Filters(',int2str(i),').MaxPhase']);
                end
                if ~isempty(selScattGainColl.Filters{1, i}.FrequencyAtMaxGain) && isnumeric(selScattGainColl.Filters{1, i}.FrequencyAtMaxGain) && isscalar(selScattGainColl.Filters{1, i}.FrequencyAtMaxGain)
            ...
            for i = 1:length(selScattGainColl.DesignOperatingCondition.MassProperties.Parameter)
                if isnumeric(selScattGainColl.DesignOperatingCondition.MassProperties.Parameter(i).Value(1,:)) && isscalar(selScattGainColl.DesignOperatingCondition.MassProperties.Parameter(i).Value(1,:))
                    CheckboxTreeNode('Name',selScattGainColl.DesignOperatingCondition.MassProperties.Parameter(i).Name,'Parent',mpNode,'Value',['DesignOperatingCondition.MassProperties.Parameter(',int2str(i),').Value']);
                end
            end

            % Inputs tree nodes
            inNode = CheckboxTreeNode('Name','Model Inputs','Parent',tree.Root,'Value','Root');
            for i = 1:length(selScattGainColl.DesignOperatingCondition.Inputs)
                if isnumeric(selScattGainColl.DesignOperatingCondition.Inputs(i).Value(1,:)) && isscalar(selScattGainColl.DesignOperatingCondition.Inputs(i).Value(1,:))
                    CheckboxTreeNode('Name',selScattGainColl.DesignOperatingCondition.Inputs(i).Name,'Parent',inNode,'Value',['DesignOperatingCondition.Inputs(',int2str(i),').Value']);
                end
            end

            % Outputs tree nodes
            outNode = CheckboxTreeNode('Name','Model Outputs','Parent',tree.Root,'Value','Root');
            for i = 1:length(selScattGainColl.DesignOperatingCondition.Outputs)
                if isnumeric(selScattGainColl.DesignOperatingCondition.Outputs(i).Value(1,:)) && isscalar(selScattGainColl.DesignOperatingCondition.Outputs(i).Value(1,:))
                    CheckboxTreeNode('Name',selScattGainColl.DesignOperatingCondition.Outputs(i).Name,'Parent',outNode,'Value',['DesignOperatingCondition.Outputs(',int2str(i),').Value']);
                end
            end
...
%             scatteredGainsCopy = obj.FilteredScatteredGains;
            ...
            selScattGainFile = obj.SelScattGainFile;
            selScattGains = selScattGainFile.getGains(obj.SelScattGainIdx);
            ...
            if ~contains(obj.IndVarExpression.AccessString,'DesignOperatingCondition') && ...
                ...
                %end
                selScattGains.DesignOperatingCondition = lacm.OperatingConditionCtrl.empty;
            else
                if ~contains(obj.IndVarExpression.AccessString,'States') && ...
                        ...
                        %end
                        selScattGains.DesignOperatingCondition.States = lacm.ConditionCtrl.empty;
                end
                if ~contains(obj.IndVarExpression.AccessString,'Inputs') && ...
                        ...
                        %end
                        selScattGains.DesignOperatingCondition.Inputs = lacm.ConditionCtrl.empty;
                end
                if ~contains(obj.IndVarExpression.AccessString,'Outputs') && ...
                        ...
                        %end
                        selScattGains.DesignOperatingCondition.Outputs = lacm.ConditionCtrl.empty;
                end
                if ~contains(obj.IndVarExpression.AccessString,'StateDerivs') && ... % Nathan added
                        ~contains(obj.GainExpression.AccessString,'StateDerivs')
                        selScattGains.DesignOperatingCondition.StateDerivs = lacm.ConditionCtrl.empty;
                end
                if ~contains(obj.IndVarExpression.AccessString,'FlightCondition') && ...
                        ...
                        %end
                        selScattGains.DesignOperatingCondition.FlightCondition = lacm.FlightCondition.empty;
                end
                if ~contains(obj.IndVarExpression.AccessString,'MassProperties') && ...
                        ...
                        %end
                        selScattGains.DesignOperatingCondition.MassProperties = lacm.MassPropertiesCtrl.empty;
                end
                %for i = 1:length(scatteredGainsCopy)
                ...
                %end
                selScattGains.DesignOperatingCondition.LinearModel = lacm.LinearModel.empty;
                selScattGains.DesignOperatingCondition.TrimSettings = lacm.TrimSettings.empty;
                selScattGains.DesignOperatingCondition.StateDerivs = lacm.Condition.empty;
                selScattGains.DesignOperatingCondition.SignalLogData = lacm.Condition.empty;
            ...
            if ~contains(obj.IndVarExpression.AccessString,'SynthesisDesignParameter') && ...
                ...
                %end
                selScattGains.SynthesisDesignParameter = ScatteredGain.Parameter.empty;
            else
                %for i = 1:length(scatteredGainsCopy)
                if contains(obj.IndVarExpression.AccessString,'SynthesisDesignParameter')
                    usedParamInVar = selScattGains.SynthesisDesignParameter.get(obj.IndVarExpression.ReplacedVariable); 
                elseif contains(obj.GainExpression.AccessString,'SynthesisDesignParameter')
                    usedParamInVar = selScattGains.SynthesisDesignParameter.get(obj.GainExpression.ReplacedVariable); 
                end
                
                selScattGains.SynthesisDesignParameter = usedParamInVar;
                %end
            end
            ...
            if ~contains(obj.IndVarExpression.AccessString,'RequirementDesignParameter') && ...
                ...
                %for i = 1:length(scatteredGainsCopy)
                selScattGains.RequirementDesignParameter = ScatteredGain.Parameter.empty;
                %end
                
            else
                %for i = 1:length(scatteredGainsCopy)
                if contains(obj.IndVarExpression.AccessString,'RequirementDesignParameter')
                    usedParamInVar = selScattGains.RequirementDesignParameter.get(obj.IndVarExpression.ReplacedVariable); 
                elseif contains(obj.GainExpression.AccessString,'RequirementDesignParameter')
                    usedParamInVar = selScattGains.RequirementDesignParameter.get(obj.GainExpression.ReplacedVariable);
                end
                selScattGains.RequirementDesignParameter = usedParamInVar;
                %end
            end 
            % Filters and below
            if ~contains(obj.IndVarExpression.AccessString,'Filters')
                ...
                %for i = 1:length(scatteredGainsCopy)
                selScattGains.Filters = UserInterface.ControlDesign.Filter.empty;
                %end
            end        
            ...
            %end
            currentGain = selScattGains.Gain.get(obj.SelectedGainString);
            selScattGains.Gain = currentGain;
            ...
            if obj.SelectedTableDimension == 1
                y = ScheduledGain.SchGainVec( ...
                                            ...
                                            obj.BreakPoints,...
                                            obj.PolyDegreeValue,...
                                            selScattGains,...%obj.FilteredScatteredGains,...
                                            obj.IncludedInFit,...
                                            obj.BreakPointsString);    
            else
                y = ScheduledGain.SchGainVec( ...
