classdef GainFilterGUI < UserInterface.Collection
    ...
    properties ( SetAccess = private )  
        FilteredGainColl ScatteredGain.GainCollectionNew = ScatteredGain.GainCollectionNew.empty
        FilteredGainCollIdx
        ScatteredGainFileObj ScatteredGain.GainFileNew = ScatteredGain.GainFileNew.empty
    end % Public properties
...
%             end  
    ...
    methods
        ...
        function data = get.TableData(obj)
            ...
            data = cell(length(obj.FilteredGainCollIdx),6);
            for i = 1:length(obj.FilteredGainCollIdx)
                data{i,1} = obj.ScatteredGainsColl.Selected(obj.FilteredGainCollIdx(i));
                [ data(i,2:5) , selFields ] = getDisplayData(...
                    obj.ScatteredGainsColl.DesignOperatingCondition,...
                    obj.FilteredGainCollIdx(i),...
                    obj.SelectedFilterFields{:});
                data{i,6} = [...
                int2str(obj.ScatteredGainsColl.Color(obj.FilteredGainCollIdx(i), 1)),',',...
                int2str(obj.ScatteredGainsColl.Color(obj.FilteredGainCollIdx(i), 2)),',',...
                int2str(obj.ScatteredGainsColl.Color(obj.FilteredGainCollIdx(i), 3))];
        ...
        function y = get.ScatteredGainsColl( obj )
            if isempty(obj.ScatteredGainFileObj)
                y = ScatteredGain.GainCollectionNew.empty;
            else
    ...
    methods 
        ...
        function resetFilter( obj )
            if isempty(obj.ScatteredGainsColl)
                obj.FilteredGainCollIdx = [];
            else
                obj.FilteredGainCollIdx = 1:obj.ScatteredGainsColl.NumGainCols;
            end
    ...
    methods (Access = protected)
        ...
        function dataUpdatedInTable( obj , hModel , hEvent , jtable )
            ...
            modifiedOC = obj.FilteredGainCollIdx(modifiedRow + 1);
            newData = hModel.getValueAt(modifiedRow,modifiedCol);
            switch modifiedCol
                case 0
                    obj.ScatteredGainsColl.Selected(modifiedOC) = newData;
                    %obj.FilteredGainCollIdx = false(1,obj.ScatteredGainsColl.NumGainCols);
                    %for i = 1:obj.ScatteredGainsColl.NumGainCols
                    %    obj.FilteredGainCollIdx(i) = obj.ScatteredGainsColl.Selected(i);
                    %end
                    %obj.FilteredGainCollIdx = find(obj.ScatteredGainsColl.Selected);
                    notify(obj,'FilteredGainsUpdated',UserInterface.UserInterfaceEventData(obj.ScatteredGainsColl));
                case 5
                    color = double([ newData.getRed , newData.getGreen , newData.getBlue ]);
                    obj.ScatteredGainsColl.Color(modifiedOC,:) = color;
            end
    ...
    methods (Access = protected)       
        ...
        function updateTable( obj , selected )%updateTable(obj)
            ...
            %switch nargin
            %    case 1
            %        selected = true(1,length(obj.FilteredGainCollIdx));
            %    case 2
            %        if ~islogical(selected) || length(selected) ~= length(obj.FilteredGainCollIdx)
            %            selected = true(1,length(obj.FilteredGainCollIdx));
            %        end    
            %end
            %for i = 1:length(obj.FilteredGainCollIdx)
            %    obj.ScatteredGainsColl.Selected(obj.FilteredGainCollIdx(i)) = selected(i);
            %end
            if ~isempty(obj.ScatteredGainsColl)
                for i = 1:obj.ScatteredGainsColl.NumGainCols
                    if ismember(i,obj.FilteredGainCollIdx)
                        obj.ScatteredGainsColl.Selected(i) = true;
                    else
                        obj.ScatteredGainsColl.Selected(i) = false;
                    end
                end
            end
...
%             end  
            ...
            if ~isempty(obj.FilteredGainCollIdx)
                notify(obj,'FilteredGainsUpdated',UserInterface.UserInterfaceEventData(obj.ScatteredGainsColl));
            end
        end % updateTable
    ...
    methods (Access = private) 
        ...
        function initializeFilteredGains(obj)
            ...
            for i = 1:length(obj.FilteredGainCollIdx)
                if ~isempty(obj.ScatteredGainsColl)
                    obj.ScatteredGainsColl.Selected(obj.FilteredGainCollIdx(i)) = true;
                    obj.ScatteredGainsColl.Color(obj.FilteredGainCollIdx(i),:) = obj.Colors{i};
                end
            end
        ...
        function updateAvaliableSelections(obj)
            if isempty(obj.ScatteredGainsColl)
                ...
                obj.SearchVar3.strList = [{obj.ScatteredGainsColl.DesignOperatingCondition.Inputs.Name}';{obj.ScatteredGainsColl.DesignOperatingCondition.Outputs.Name}'];
                obj.SearchVar3.selVal = 1; 
                ...
                gainCollAll = 1:obj.ScatteredGainsColl.NumGainCols;
 
                obj.JCBListArraySearchValue1 = obj.getAvaliableSelectionsFC( gainCollAll , obj.SearchVar1.selStr );%obj.JCBListArraySearchValue1 = obj.getAvaliableSelectionsFC( filteredGainColl , obj.SearchVar1.selStr );
                ...
                obj.JCBListArraySearchValue2 = obj.getAvaliableSelectionsFC( gainCollAll , obj.SearchVar2.selStr );%obj.JCBListArraySearchValue2 = obj.getAvaliableSelectionsFC( filteredGainColl , obj.SearchVar2.selStr );
                ...
                obj.JCBListArraySearchValue3 = obj.getAvaliableSelectionsIC( gainCollAll , obj.SearchVar3.selStr );%obj.JCBListArraySearchValue3 = obj.getAvaliableSelectionsIC( filteredGainColl , obj.SearchVar3.selStr );
                ...
                obj.JCBListArraySearchValue4 = obj.getAvaliableSelectionsWC( gainCollAll , obj.SearchVar4.selStr );%obj.JCBListArraySearchValue4 = obj.getAvaliableSelectionsWC( filteredGainColl , obj.SearchVar4.selStr );
        ...
        function y = getAvaliableSelectionsFC( obj ,  GainCollIdx , selStr )
            if ~strcmp(selStr,'All')
                sl1 = zeros(1, length(GainCollIdx));
                for i = 1:length(GainCollIdx)
                    sl1(i) = obj.ScatteredGainsColl.DesignOperatingCondition.FlightCondition(GainCollIdx(i)).(selStr);
                end
        ...
        function y = getAvaliableSelectionsIC( obj , GainCollIdx , selStr )
            if ~strcmp(selStr,'All')
                sl1 = zeros(1, length(GainCollIdx));
                for i = 1:length(GainCollIdx)
                    try
                        sl1(i) = obj.ScatteredGainsColl.DesignOperatingCondition.Inputs.get(selStr).Value(GainCollIdx(i));
                    catch
                        sl1(i) = obj.ScatteredGainsColl.DesignOperatingCondition.Outputs.get(selStr).Value(GainCollIdx(i));
                    end
                end
        ...
        function y = getAvaliableSelectionsWC( obj , GainCollIdx , selStr )
            if ~strcmp(selStr,'All')
                sl4 = cell(1, length(GainCollIdx));%sl4 = zeros(size(FilteredGainColl4));
                for i = 1:length(GainCollIdx)
                    sl4{i} = num2str(obj.ScatteredGainsColl.DesignOperatingCondition.MassProperties.get(selStr, GainCollIdx(i)));
                end
...
%             javaaddpath(fileparts(mfilename('fullpath')));  % add the Java class file to the dynamic java class-path
            ...
            updateTable( obj ); % Nathan commented out, this is called inside of update
            resize( obj , [] , [] );
    ...
    methods
        ...
        function searchValue1_pm_CB( obj , ~ , ~ )
            ...
            obj.JCBListArraySearchValue2 = obj.getAvaliableSelectionsFC( filteredGainCollIdx , obj.SearchVar2.selStr );
            obj.JCBListSelectedStringSearchValue2 = obj.JCBListArraySearchValue2;


            obj.JCBListArraySearchValue3 = obj.getAvaliableSelectionsIC( filteredGainCollIdx , obj.SearchVar3.selStr );
            obj.JCBListSelectedStringSearchValue3 = obj.JCBListArraySearchValue3;


            obj.JCBListArraySearchValue4 = obj.getAvaliableSelectionsWC( filteredGainCollIdx , obj.SearchVar4.selStr );
        ...
        function searchVar1_pm_CB( obj , hobj , ~ )
            ...
            [filteredGainCollIdx,~] = searchScatteredGainsDesignCond(obj.ScatteredGainsColl,...
            ...
            obj.JCBListArraySearchValue1 = obj.getAvaliableSelectionsFC( filteredGainCollIdx , obj.SearchVar1.selStr );
...
function [dataIdx,dataNames] = searchScatteredGainsDesignCond(data, fc1, fc1Val, fc2, fc2Val, ic, icVal, wc, wcStr, err)
...
if isempty(data)
    ...
    operCond = data.DesignOperatingCondition;
end

if isempty(fc1Val)
    fc1LogInd = true(operCond.NumConds, 1);
else
    fc1Val = str2double(fc1Val);
    fc1LogInd = false(operCond.NumConds, 1);
    for ind = 1:length(fc1Val)
        ...
        for i = 1:length(fc1LogInd)
            fc1LogInd(i) = fc1LogInd(i) || (round2(operCond.FlightCondition(i).(fc1),err) == round2(fc1Val(ind), err));
        end
    end
end

if isempty(fc2Val)
    fc2LogInd = true(operCond.NumConds, 1);
else
    fc2Val = str2double(fc2Val);
    fc2LogInd = false(operCond.NumConds, 1);
    for ind = 1:length(fc2Val)
        ...
        for i = 1:length(fc1LogInd)
            fc2LogInd(i) = fc2LogInd(i) || (round2(operCond.FlightCondition(i).(fc2),err) == round2(fc2Val(ind), err));
        end
    end
end

if isempty(icVal)
    icLogInd = true(operCond.NumConds, 1);
else
    icVal = str2double(icVal);
    icLogInd = false(operCond.NumConds, 1);
    
    
     if ~isempty(operCond) && ~isempty(operCond.Inputs.get(ic)) 
        for ind = 1:length(icVal)
            ...
            for i = 1:length(icLogInd)
                icLogInd(i) = icLogInd(i) || (round2(operCond.Inputs.get(ic).Value(i),err) == round2(icVal(ind), err));
            end
        end
     else
        for ind = 1:length(icVal)
            ...
            %icLogInd = or(icLogInd,icLogCell{ind});
            for i = 1:length(icLogInd)
                icLogInd(i) = icLogInd(i) || (round2(operCond.Outputs.get(ic).Value(i),err) == round2(icVal(ind), err));
            end
        end
     end
end
...
if isempty(wcStr) || (iscell(wcStr) && length(wcStr) == 1 && isempty(wcStr{1}))
    wcLogInd = true(operCond.NumConds, 1);
else
    if ischar(wcStr);wcStr = {wcStr};end;
    wcVal = str2double(wcStr);
    wcLogInd = false(operCond.NumConds, 1);
    for ind = 1:length(wcVal)
        if strcmp(wc,'Label') && isnan(wcVal(ind))
            ...
            for i = 1:length(wcLogInd)
                wcLogInd(i) = wcLogInd(i) || strcmp(operCond.MassProperties.get(wc, i), wcStr{ind});
            end
        elseif strcmp(wc,'Label') && ~isnan(wcVal(ind))
            ...
            for i = 1:length(icLogInd)
                wcLogInd(i) = wcLogInd(i) || (round2(operCond.MassProperties.get(wc, i),err) == round2(wcVal(ind), err));
            end
...
dataIdx = find(fc1LogInd & fc2LogInd & icLogInd & wcLogInd);
