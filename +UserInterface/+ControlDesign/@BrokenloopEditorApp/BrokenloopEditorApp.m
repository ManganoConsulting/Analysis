classdef BrokenloopEditorApp < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function focusUIFigure(app)
            % Bring the UIFigure to focus in all MATLAB versions
    ...
    methods (Access = private)
        ...
        function initOnMdlLoad(app)
            ...
            if length(inputNames) ~= length(outputNames)
                msgbox("The Simulink model must contain an equal number of inputs and outputs.", "Error", "error");
                app.SimulinkModelEdit.Value = [];
                return;
            end


            app.PlotTitles = cell(length(inputNames), 1);
            app.SelectedPlots = repmat({true}, length(inputNames), 3);
            app.ObjectTitles = {};
            for i = 1:length(inputNames)
                if app.UseCommonName
                    title = longestCommonSubstring(inputNames{i}, outputNames{i});
                    app.PlotTitles{i} = ['Broken Loop at ', title];
                else
                    app.PlotTitles{i} = ['Broken Loop at ', inputNames{i}, ' to ', outputNames{i}];
                end

                app.ObjectTitles = [app.ObjectTitles; [app.PlotTitles{i}, ' - Bode Mag']; [app.PlotTitles{i}, ' - Bode Phase']; [app.PlotTitles{i}, ' - Nichols']];
            end

            app.DataTable.Data = [app.PlotTitles, app.SelectedPlots];
            app.OrderTable.Data = app.ObjectTitles;

            result = generateCustomPattern(length(inputNames));
            app.OutputDataIndexArray = result;

            app.ObjectName2OutputDataInd = [app.OrderTable.Data, num2cell(app.OutputDataIndexArray)'];
        end

        function objs = createStabObjs(app)
            [~, simMdl] = fileparts(app.SimulinkModelEdit.Value);
            [~, methodName] = fileparts(app.MethodFileName);

            objs(length(app.OrderTable.Data)) = Requirements.Stability;
            for i = 1:length(app.OrderTable.Data)
                ...
                objs(i) = Requirements.Stability(methodName, app.OrderTable.Data{i}, simMdl);
...
%                 app.(currCol).Text = '▣';
        ...
        function onLoad(app, ~)
            ...
            create_olfr_file(app, filepath);
            
            objs = createStabObjs(app);
        ...
        function onExportObjects(app, ~)
            ...
            objs = createStabObjs(app);outputFile
    ...
    methods (Access = public)
        ...
        % Construct app
        function app = BrokenloopEditorApp
    ...
    methods (Access = private)
        function createComponents(app)
            ...
            app.UIFigure = uifigure('Name', 'Brokenloop Editor');
            ...
            uilabel(t1, 'Text', ' Brokenloop Editor', 'FontWeight', 'bold', 'FontSize', 16,...
            ...
            % --- Common Name Check Box ---
%             app.CommonNameCheck = uicheckbox(brknLoopPanel, 'Text', 'Use Common Name', 'Value', app.UseCommonName, ...
%                 'ValueChangedFcn', @(s, e) onCommNameCheck(app, s, e),...
...
%             app.SelectAllMag = uibutton(brknLoopPanel, 'state', ...
%                 'Text', '☑', ...
%                 'FontSize', 14,...
%                 'FontWeight', 'Bold', ...
%                 'ValueChangedFcn', @(s, e) selectDeselectAllColumns(app, s, e, 'mag'),...
%                 'Position', [335 5 50 20]);
% 
%             app.SelectAllPhase = uibutton(brknLoopPanel, 'state', ...
%                 'Text', '☑', ...
%                 'FontSize', 14,...
%                 'FontWeight', 'Bold', ...
%                 'ValueChangedFcn', @(s, e) selectDeselectAllColumns(app, s, e, 'phase'),...
%                 'Position', [417 5 50 20]);
% 
%             app.SelectAllNick = uibutton(brknLoopPanel, 'state', ...
%                 'Text', '☑', ...
%                 'FontSize', 14,...
%                 'FontWeight', 'Bold', ...
%                 'ValueChangedFcn', @(s, e) selectDeselectAllColumns(app, s, e, 'nick'),...
%                 'Position', [490 5 50 20]);
            % Order listbox
