...
%         ScattteredGainColl ScatteredGain.GainCollection
        ...
        SelectedScatteredGainFileObj ScatteredGain.GainFileNew  
    ...
    properties
        ...
        ScattGainFileObjArray = ScatteredGain.GainFileNew.empty
...
%                 addlistener(obj.Tree,'LoadedSchGainObj','PostSet',@obj.setSchGainFileComboBox);
                ...
                addlistener(obj.Tree,'ClearSimPlots',@obj.clearSimPlots);
...
%                 selDesignOperCond = obj.OperCondColl.SelDesignOperCond;
                selDesignOperCond = obj.OperCondColl.getOperCondLeg( obj.OperCondColl.SelDesignOperCondIdx );
                ...
                selDesignOperCondsAll = lacm.OperatingCondition.empty;
                for ocIdx = 1:scattGainObjArray.DesignOperatingCondition.NumConds
                    selDesignOperCondsAll(ocIdx) = Utilities.operCondCtrlToLeg(scattGainObjArray.DesignOperatingCondition, ocIdx);
                end
...
%                 end
                ...
                if length(selDesignOperCond) > 1
                    ...
                    for i = 1:length(selDesignOperCond)
%                             logArray = logArray | [scattGainObjArray.DesignOperatingCondition] == selDesignOperCond(i);
                        logArray = logArray | selDesignOperCondsAll == selDesignOperCond(i);
                    end
                else
%                         logArray = Utilities.operCondCtrlToLeg(scattGainObjArray.DesignOperatingCondition) == selDesignOperCond;
%                     logArray = selDesignOperCondsAll == selDesignOperCond;
                    logArray = false;
                    for selidx = 1:length(selDesignOperCondsAll)
                        logArray = logArray || Utilities.operCondCompare(selDesignOperCondsAll(selidx), selDesignOperCond);
                    end
                end
...
%                 simreqObjCopy.SimViewerProject = struct('SimulationData',{},'PlotSettings',{},'RunLabel',{},'TreeExpansionState',{},'RunSpecificColors',{});
            ...
            scatteredGainColl = getCurrentScatteredGainObj( obj , obj.ReqParamColl , obj.FilterParamColl , ...
                obj.OperCondColl.getOperCondLeg( obj.OperCondColl.SelDesignOperCondIdx ), ...
            ...
            selAnalysis = obj.OperCondColl.AnalysisBool;
            
            indices = obj.OperCondColl.SelAnalysisOperCondIdx;
            numOperCond = length(indices);
            operCondAnalysis(numOperCond) = lacm.OperatingCondition;
            for i = 1:numOperCond
                [setIdx, ocIdx] = obj.OperCondColl.getOcIdx(indices(i));
                operCondAnalysis(i) = Utilities.operCondCtrlToLeg(obj.OperCondColl.OperatingConditionCell{setIdx}, ocIdx);
            end
            
            operCondDesign = obj.OperCondColl.getOperCondLeg( obj.OperCondColl.SelDesignOperCondIdx );
            ...
            batchRunObjects = UserInterface.ControlDesign.RunObject( 'StabReqObj',stabreqObjCopy,...
                                                ...
                                                'AnalysisOperCond',operCondAnalysis,...
                                                'DesignOperCond',operCondDesign,...
...
%                     obj.RunSelJButton.setFlyOverAppearance(true);
    ...
    methods (Access = protected) 
        ...
        function saveToolRibbon_CB( obj , ~ , ~ )
            ...
            menuItem3 = javaObjectEDT('javax.swing.JMenuItem','<html>Save Project As',saveWorkspaceIcon);
            menuItem3h = handle(menuItem3,'CallbackProperties');
            set(menuItem3h,'ActionPerformedCallback',@obj.saveWorkspaceAs); 
...
%             jmenu.add(menuItem4);
        ...
        function clearSimPlots(obj, hobj, eventdata)
           clearAllPlots_CB( obj.SimViewerColl , [] , [] )
        end
    end
...
%             for i = 1:length(allFilteredOC)
            if strcmp('all',selected)
                ...
                logArray = obj.OperCondColl.AnalysisBool;
...
%                 Utilities.write2mfile( obj.OperCondColl.FilteredOperConds(logArray) , true , headerData , tableData(logArray,:) );
                Utilities.write2mfile( operCondAnalysis , true , headerData , tableData(logArray,:) );
                drawnow();pause(0.1);
            end
%             end
...
%             appendRun2WordDocument( obj , saveFigs , rpt , obj.OperCondColl.FilteredOperConds([obj.OperCondColl.FilteredOperConds.SelectedforAnalysis]) , charAnswer );
            appendRun2WordDocument( obj , saveFigs , rpt , operCondAnalysis , charAnswer );
...
%         end % newOperCond
        ...
        function menuScattGain( obj , ~ , ~ )
            insertScatteredGainCollObjFile_CB( obj.Tree , [] , [] , obj.Tree.GainsScattered , [] , ScatteredGain.GainFileNew.empty);
                ...
                if obj.OperCondTabPanel.SelectedTab == obj.BatchTab
                    ...
                    if ~isempty(obj.CurrentScatteredGainColl) && ~isempty(obj.CurrentScatteredGainColl.DesignOperatingCondition)% && ...
                        ...
                        if sourceGainSelected( obj.Tree ) 
                            ...
                            if ~isempty(obj.OperCondColl.SelDesignOperCondIdx ) % Nathan changed
    ...
    methods 
        ...
        function saveWorkspaceAs( obj , ~ , ~ )
            ...
            if isequal(file,0) || isequal(path,0)
                ...
                answer = questdlg('Would you like to save the plots?', ...
                    'Save Plots', ...
                    'Yes','No','No'); 
                
                switch answer
                    case 'Yes'
                        notify(obj,'SaveProject',UserInterface.SaveProjectEventData({path , file}, true));
                    case 'No'
                        notify(obj,'SaveProject',UserInterface.SaveProjectEventData({path , file}, false));
                    otherwise
                        return;
                end
            end
...
%                     saveProject( obj , [] , saveFIG.PLTresponse);
                    notify(obj,'SaveProject',UserInterface.SaveProjectEventData([], saveFIG.PLTresponse));
                otherwise
                    % Do nothing
            end
        end % saveWorkspace
...
%                     selDesignOperCondsAll = selDesignOperCond;
                    selDesignOperCondsAll = lacm.OperatingCondition.empty;
                    for ocIdx = 1:scattGainObjArray.DesignOperatingCondition.NumConds
                        selDesignOperCondsAll(ocIdx) = Utilities.operCondCtrlToLeg(scattGainObjArray.DesignOperatingCondition, ocIdx);
                    end
...
%                         logArray = Utilities.operCondCtrlToLeg(scattGainObjArray.DesignOperatingCondition) == selDesignOperCond;
                        ...
                        logArrayIdx = 0;
                        for selidx = 1:length(selDesignOperCondsAll)
                            if Utilities.operCondCompare(selDesignOperCondsAll(selidx), selDesignOperCond)
                                logArray = true;
                                logArrayIdx = selidx;
                                break;
                            end
...
%                         logArrayIdx = find(logArray);
                        scatGainCollObjOperConds = lacm.OperatingCondition.empty;
%                         for logItr = 1:length(logArrayIdx)
%                             scatGainCollObjOperConds(ocIdx) = Utilities.operCondCtrlToLeg(scattGainObjArray.DesignOperatingCondition, logArrayIdx(logItr));
%                         end
                        scatGainCollObjOperConds = Utilities.operCondCtrlToLeg(scattGainObjArray.DesignOperatingCondition, logArrayIdx);
                        scattGainLeg = Utilities.scattGainCollCtrlToLeg(scattGainObjArray, logArrayIdx);
                        scatGainCollObj = ScatteredGain.GainCollection( 'OperatingCondition', scatGainCollObjOperConds ,...
                                                         'Gains' , scattGainLeg.Gain ,...
                                                         'SynthesisDesignParameter' , scattGainLeg.SynthesisDesignParameter ,...
                                                         'RequirementDesignParameter' , scattGainLeg.RequirementDesignParameter ,...
                                                         'Filters', scattGainObjArray.Filters{logArrayIdx},... % Taking the first one might not be sufficient, as it is the most recent one that might be more important
                                                         'OperCondFilterSettings', scattGainObjArray.OperCondFilterSettings(logArrayIdx));
%                         scatGainCollObj = scattGainObjArray(logArray);
                    end
        ...
        function run( obj , ~ , ~ )
            ...
            % Nathan changed
            obj.CurrentScatteredGainColl = getCurrentScatteredGainObj( obj , obj.ReqParamColl , obj.FilterParamColl , ...
                obj.OperCondColl.getOperCondLeg( obj.OperCondColl.SelDesignOperCondIdx ), ...
                getSelectedSynthesisObjs(obj.Tree) , obj.SynthesisParamColl);

            obj.OperCondColl.LastSelectedDesignCond    = obj.OperCondColl.DesignBool;
            obj.OperCondColl.LastSelectedAnaylysisCond = obj.OperCondColl.AnalysisBool;
            ...
            runRequirementSelected( obj , operCondAnalysis , obj.CurrentScatteredGainColl );
        ...
        function runRequirementSelected( obj , analysisOperCond , scattGainCollObj )
            ...
            if ~isempty(reqObjs)
                ...
                for i = 1:length(reqObjs)
                    indices = obj.OperCondColl.SelAnalysisOperCondIdx;
                    indicesFilt = find(obj.OperCondColl.AnalysisBool);
                    numOperCond = length(indices);
                    operCondAnalysis(numOperCond) = lacm.OperatingCondition;
                    for j = 1:numOperCond
                        [setIdx, ocIdx] = obj.OperCondColl.getOcIdx(indices(j));
                        operCondAnalysis(j) = Utilities.operCondCtrlToLeg(obj.OperCondColl.OperatingConditionCell{setIdx}, ocIdx);
                    end
%                     selAnalysis = [obj.OperCondColl.FilteredOperConds.SelectedforAnalysis];
%                     allFilterColors = {obj.OperCondColl.FilteredOperConds.Color};
                    allAnalysisColors = obj.OperCondColl.getAnalysisOperCondColors;
                    ...
                    if ~isempty(indices)
                        updateNewData(obj.SimViewerColl(simViewInd(i)) ,reqObjs(i).SimulationData.Output , false , 1, obj.OperCondColl.SelectedDisplayData(indicesFilt,2), allAnalysisColors);
                    end
                    reqObjs(i).AnalysisOperCondDisplayText = obj.OperCondColl.SelectedDisplayData(indicesFilt,2);
                    reqObjs(i).AnalysisOperCond = copy(operCondAnalysis);
