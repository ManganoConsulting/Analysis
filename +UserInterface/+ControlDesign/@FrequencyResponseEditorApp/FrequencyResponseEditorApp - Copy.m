...
%                 [inputNames, outputNames] = UserInterface.ControlDesign.FrequencyResponseEditorApp.findSimulinkIO(app, mdlName);
                [inputNames, outputNames] = findSimulinkIO(app, mdlName);
            end
...
%             app.PlotTitles = cell(length(inputNames), 1);
            app.SelectedPlots = repmat({true}, length(app.PlotTitles), 2);
            app.ObjectTitles = {};
            for i = 1:size(app.PlotTitles,1)
                app.ObjectTitles = [app.ObjectTitles; [app.PlotTitles{i}, ' - Bode Mag']; [app.PlotTitles{i}, ' - Bode Phase']];
            end
