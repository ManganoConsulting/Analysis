...
%         ScatteredGainObjReplaced
    ...
    methods
        ...
        function create( obj , Params ) 
            ...
            obj.TreeObj = CheckboxTree('Parent',parent,'MouseClickedCallback',@obj.mousePressedInTree_CB);
            obj.TreeObj.RootVisible= false;
            
            % Create tree nodes
            obj.OperCondNode = CheckboxTreeNode('Name','Operating Condition','Parent',obj.TreeObj.Root,'CheckboxVisible',false,'Value',1);
            
            obj.GainsNode = CheckboxTreeNode('Name','Gain Source','Parent',obj.TreeObj.Root,'CheckboxVisible',false,'Value',1000);
                obj.GainsSynthesis = CheckboxTreeNode('Name','Synthesis','Parent',obj.GainsNode,'CheckboxVisible',true,'Value',2);
                obj.GainsScattered = CheckboxTreeNode('Name','Scattered','Parent',obj.GainsNode,'CheckboxVisible',true,'Value',3);
                obj.GainsScheduled = CheckboxTreeNode('Name','Scheduled','Parent',obj.GainsNode,'CheckboxVisible',true,'Value',4);
                obj.RootLocusNode = CheckboxTreeNode('Name','Root Locus','Parent',obj.GainsNode,'CheckboxVisible',true,'Value',5);
            
            obj.ReqNode = CheckboxTreeNode('Name','Requirements','Parent',obj.TreeObj.Root,'CheckboxVisible',true,'Value',2000);
                obj.StabilityReqNode = CheckboxTreeNode('Name','Stability','Parent',obj.ReqNode,'CheckboxVisible',true,'Value',6);
                obj.FreqNode = CheckboxTreeNode('Name','FrequencyResponse','Parent',obj.ReqNode,'CheckboxVisible',true,'Value',7);
                obj.SimNode = CheckboxTreeNode('Name','Simulation','Parent',obj.ReqNode,'CheckboxVisible',true,'Value',8);
                obj.HQNode = CheckboxTreeNode('Name','HandlingQualities','Parent',obj.ReqNode,'CheckboxVisible',true,'Value',9);
                obj.ASENode = CheckboxTreeNode('Name','Aeroservoelasticity','Parent',obj.ReqNode,'CheckboxVisible',true,'Value',10);   
        end % create  
        ...
        function restore( obj , Params ) 
            ...
            TreeNodes = CheckboxTreeNode.empty;
            ...
            for i = 2:length(nodeData)
                ...
                node = CheckboxTreeNode('Parent',parentNode,...
                        'Name',nodeData(i).Name,...
                        'TooltipString',nodeData(i).TooltipString,...
                        'Checked',nodeData(i).Checked,...
                        'CheckboxEnabled',nodeData(i).CheckboxEnabled,...
                        'CheckboxVisible',nodeData(i).CheckboxVisible,...
                        'Value',nodeData(i).Value,...
                        'UserData',nodeData(i).UserData,...
                        'UIContextMenu',nodeData(i).UIContextMenu);
                if ~isempty(nodeData(i).Icon)
                    try
                        % find the icon Resources directory
                        this_dir = fileparts( mfilename( 'fullpath' ) );
                        expression = '+UserInterface\\.+';
                        matchStr = regexp(char(nodeData(i).Icon),expression,'match');     
                        icon_dir = fullfile( this_dir,'..','..','..',matchStr);
                        %isfile(icon_dir{:})

                        % Set the Icon
                        node.setIcon(icon_dir{:});   
                    catch
                        warning('Unable to set icon');
                    end
                end
    ...
    methods
        ...
        function rObjs = getSelectedSynthesisObjs( obj )
            ...
            if ~isempty(obj.GainsSynthesis.Children)
                logArray = [obj.GainsSynthesis.Children.Checked];
        ...
        function rObjs = getSelectedRLocusObjs( obj )
            ...
            if ~isempty(obj.RootLocusNode.Children)
                logArray = [obj.RootLocusNode.Children.Checked];
        ...
        function rObjs = getSelectedReqObjs( obj , type )
            ...
            if ~isempty(obj.(type).Children)
                ...
                for i = 0:obj.(type).getJavaObjects.jNode.getChildCount - 1
                    jNames{i+1} = char(obj.(type).getJavaObjects.jNode.getChildAt(i).toString);
                    mNames{i+1} = children(i+1).Name;
                end
                [~, idx] = ismember(jNames, mNames);

                logArray = [obj.(type).Children.Checked];
                logArray = logArray(idx);
                tempObj = obj.(type).Children(idx); % Reorder objs to correct order
                rObjs = [tempObj(logArray).UserData];
        ...
        function rObjs = getSelectedSourceGainObjs(obj)
            ...
            if ~isempty(obj.GainsScheduled.Children)
                logArray = [obj.GainsScheduled.Children.Checked];
        ...
        function rObjs = getSelectedScatteredGainObjs(obj)
            ...
            if ~isempty(obj.GainsScattered.Children)
                logArray = [obj.GainsScattered.Children.Checked];
...
%                 (obj.GainsScheduled.Checked || obj.GainsScheduled.PartiallyChecked) && ~isempty(obj.GainsScheduled.Children)];       
            x = [(obj.GainsSynthesis.Checked || obj.GainsSynthesis.PartiallyChecked),...
                (obj.GainsScattered.Checked || obj.GainsScattered.PartiallyChecked),...
                (obj.GainsScheduled.Checked || obj.GainsScheduled.PartiallyChecked),...
                (obj.RootLocusNode.Checked || obj.RootLocusNode.PartiallyChecked)];
            if any(x)
...
%             rlSel = (obj.RootLocusNode.Checked || obj.RootLocusNode.PartiallyChecked) && ~isempty(obj.RootLocusNode.Children);
            rlSel = (obj.RootLocusNode.Checked || obj.RootLocusNode.PartiallyChecked);
        end % getGainSource
    ...
    methods
        ...
        function addOperCond(obj, ~ , ~ , parentNode )
            ...
            if ~iscell(filename)
                ...
                for k = 1:length(filename)
                    ...
                    node = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',filename{k},'Parent',parentNode,'Checked',true,'CheckboxVisible',false,'Value',11);     
                end
        ...
        %----------------------------------------------------------------------
            ...
            node = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',answer,'Parent',parentNode,'Checked',true,'CheckboxVisible',false,'Value',100);
            node.setIcon(getIcon('Open_16.png'));
...
%             index = length(reqObj);
            for index = 1:length(reqObj)
                ...
                node = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',reqObj(index).Title,'Parent',parentNode,'Checked',check,'CheckboxVisible',true,'Value',value);      
    
                if ~isempty(reqObj(index).MdlName)
                    modelNode = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',reqObj(index).MdlName,'Parent',node,'Checked',check,'CheckboxVisible',false,'Value',19);
                    modelNode.setIcon(getIcon('Simulink_16.png'));
                    
                end
                if ~isempty(reqObj(index).FunName)     
                    methodNode = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',reqObj(index).FunName,'Parent',node,'Checked',check,'CheckboxVisible',false,'Value',20);
                    methodNode.setIcon(getIcon('new_script_ts_16.png'));
                ...
                if (isa(reqObj,'Requirements.RequirementTypeOne') && ~isempty(reqObj(index).RequiermentPlot)) ||  (isa(reqObj,'Requirements.RootLocus') && ~isempty(reqObj(index).RequiermentPlot))         
                    methodNode = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',reqObj(index).RequiermentPlot,'Parent',node,'Checked',check,'CheckboxVisible',false,'Value',30);
                    methodNode.setIcon(getIcon('Figure_16.png'));
                ...
                if isa(reqObj,'Requirements.Synthesis') %& ~isa(reqObj,'Requirements.RootLocus')
                    ...
                    if isempty(obj.SelectedScatteredGainFileObj )
                        scatteredNode = UserInterface.uiextras.jTree.CheckboxTreeNode('Name','','Parent',node,...
                            'Checked',true,'CheckboxVisible',false,'Value',22,'UserData',[]);
                        scatteredNode.setIcon(getIcon(obj.ScattGainIcon)); 
                        ...
                        scatteredNode = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',obj.SelectedScatteredGainFileObj.Name,'Parent',node,...
                            'Checked',true,'CheckboxVisible',false,'Value',22,'UserData',[]);
                        scatteredNode.setIcon(getIcon(obj.ScattGainIcon)); 
                    end
    ...
    methods 
        ...
        %MOVETREENODE Move a jTree node up or down among its siblings
        %
        % node:      a CheckboxTreeNode
        % direction: 'up' or 'down'
            
            parent = node.Parent();
            jNode = node.getJavaObjects.jNode;
            jParent = parent.getJavaObjects.jNode;
            if isempty(parent)
                error('Cannot move the root node.');
            end
        
            % Get current index (Java-based)
            idx = jParent.getIndex(jNode);  % 0-based index
            childCount = jParent.getChildCount();
            ...
            jParent.remove(idx);
            jParent.insert(jNode, newIdx);

            treeModel = obj.TreeObj.getJavaObjects.jTree.getModel();
            treeModel.reload(jParent);
            ...
            if ~iscell(filename)
                ...
                for k = 1:length(filename)
                    ...
                    for i = 1:length(varNames)
                        ...
                        node = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',reqObj(index).Name,'Parent',parentNode,'Checked',false,'CheckboxVisible',true,'Value',40);    
                        ...
                        for j = 1:length(allGainNames)
                            if any(strcmp(allGainNames{j},completedGainNames))
                                ...
                                node = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',['<html><font color="red"><i>' allGainNames{j}],'Parent',parentNode,'Checked',false,'CheckboxVisible',false,'Value',41);  

                            else
                                node = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',allGainNames{j},'Parent',parentNode,'Checked',false,'CheckboxVisible',false,'Value',42);  
                            end 
                            
                            
                            m=j;
                            
                            node1 = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',[reqObj(index).Gain(m).BreakPoints1Name,' - ',num2str(reqObj(index).Gain(m).Breakpoints1Values)],...
                                'Parent',node,'Checked',false,'CheckboxVisible',false,'Value',43);  
                            
                            if ~isempty(reqObj(index).Gain(m).BreakPoints2Name)
                                node2 = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',[reqObj(index).Gain(m).BreakPoints2Name,' - ',num2str(reqObj(index).Gain(m).Breakpoints2Values)],...
                                    'Parent',node,'Checked',false,'CheckboxVisible',false,'Value',44);  
                            end
                            node3 = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',['TableData - ',[num2str(size(reqObj(index).Gain(m).TableData,1)),' x ',num2str(size(reqObj(index).Gain(m).TableData,2))]],...
                                'Parent',node,'Checked',false,'CheckboxVisible',false,'Value',45);
...
%                 node.setIcon(getIcon(obj.ScattGainIcon)); 
        ...
        function insertEmptyScatteredGainObj_CB( obj , ~ , ~ , synNode , set2saved )
            ...
            node = UserInterface.uiextras.jTree.CheckboxTreeNode('Name',gainScattCollName,'Parent',obj.GainsScattered,...
                'Checked',false,'CheckboxVisible',true,'Value',21,'UserData',tempScattGainFile);
            node.setIcon(getIcon(obj.ScattGainIcon));
