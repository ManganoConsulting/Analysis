function mdlParams = assignParameters2Model( mdlName , mdlParams )
    ...
    if ~isempty(wrkspace)
        ...
        if isa(mdlParams,'ScatteredGain.GainCollection')
            ...
            for i = 1:length(mdlParams.RequirementDesignParameter)  
                wrkspace.assignin(mdlParams.RequirementDesignParameter(i).Name,mdlParams.RequirementDesignParameter(i).Value);%(mdlParams.DesignParameter(i).Name,mdlParams.DesignParameter(i).Value);
            end
            % Assign the Gains to the model
            for i = 1:length(mdlParams.Gain)  
                wrkspace.assignin(mdlParams.Gain(i).Name,mdlParams.Gain(i).Value);
            end
