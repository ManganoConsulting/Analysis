function [out,divOut] = find_in_models(modelname,varargin)
...
% Copyright 2008-2010 The MathWorks, Inc.
    ...
    % Run the search.
    [out,divOut] = i_search(modelname,varargin{:});
    ...
    function [out,divOut] = i_search(thismodel,varargin)
        ...
        % Get Data Logging Signals 
        ph = get_param(out,'SrcPortHandle');
        signals = struct('SignalName',{},'BusObject',{},'Children',{});
        for i=1: length(ph)
            state = get_param(ph{i},'datalogging');
            if strcmp(state,'on')
                signals(end + 1) = get_param(ph{i},'SignalHierarchy'); %#ok<AGROW>
            end
        end

        divOut.(thismodel) = struct('Signals',out,'MdlRef',struct,'DataLoggingSignals',signals);
...
%                     divOut.MdlRef = submodel;
                    % Now search the model.
                    [others{i},out2] = i_search(submodel,varargin{:});
                    
%                     divOut.(thismodel).MdlRef(end+1) = out2;
                    fnames = fieldnames(out2);
                    for ind = 1:length(fnames)
                        divOut.(thismodel).MdlRef.(fnames{ind}) = out2.(fnames{ind});
                    end
