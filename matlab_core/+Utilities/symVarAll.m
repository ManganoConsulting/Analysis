...
%   $Revision: 1.13.4.6 $ $Date: 2010/08/23 23:09:17 $
...
if any(row)
  % Make the matrix, convert it to a padded string matrix
  % and then a cell array of strings.
  vars = accumarray([row(row~=0).' col(col~=0).'],s(logical(mask)).')';
  vars(vars==0) = ' ';
...
if ~isempty(q)
  if rem(length(q),2)~=0,
    error(message('MATLAB:symvar:SQuotesNotPaired'))
  end
