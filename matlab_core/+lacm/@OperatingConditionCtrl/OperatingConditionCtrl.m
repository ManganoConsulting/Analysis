classdef OperatingConditionCtrl < matlab.mixin.Copyable%< matlab.mixin.CustomDisplay
    ...
    properties  
        ...
        ModelName string
        NumConds
        States lacm.ConditionCtrl
        %StateVals single
        Inputs lacm.ConditionCtrl
        %InputVals single
        Outputs lacm.ConditionCtrl
        %OutputVals single
        StateDerivs lacm.ConditionCtrl
        %StateDerivVals single
        FlightCondition lacm.FlightCondition
        MassProperties lacm.MassPropertiesCtrl
        LinearModel lacm.LinearModel 
        TrimSettings lacm.TrimSettings % legacy 
        SignalLogData lacm.Condition % legacy
        %SignalLogDataVals single
        SaveFormat % legacy
        SuccessfulTrim logical
        Cost single
        SelectedforAnalysis logical = [false]
        SelectedforDesign logical = [false]
        %IncorrectTrimText = string;
    end % Public properties
        
    %% Read-only properties
    properties ( GetAccess = public, SetAccess = private )
        %SuccessfulTrim double
        %Cost double
        %IncorrectTrimText = string;
    ...
    methods   
        ...
        %function obj = OperatingConditionCtrl(varargin)
        %    if nargin == 0
        %        obj = lacm.OperatingConditionCtrl;
        %        obj.MassProperties = lacm.MassPropertiesCtrl;
        %        obj.NumConds = 0;
        %        return;
        ...
        %        obj(numOfTrims) = lacm.OperatingConditionNew; % Initialize 
        %        for i = 1:numOfTrims
...
%             end
    ...
    methods 
        ...
        % Nathan added, for scattered gains functionality
        function addOperCond( obj, operCondNew )
            if isempty(obj.NumConds)
                obj.NumConds = 1;
                obj.setOperCondStruct(operCondNew);
            else
                obj.NumConds = obj.NumConds + 1;
            end
            obj.Label(obj.NumConds) = operCondNew.Label;
            obj.ModelName(obj.NumConds) = operCondNew.ModelName;
            for i = 1:length(operCondNew.States)
                obj.States(i).Value(obj.NumConds,:) = single(operCondNew.States(i).Value);
            end
            for i = 1:length(operCondNew.Inputs)
                obj.Inputs(i).Value(obj.NumConds,:) = single(operCondNew.Inputs(i).Value);
            end
            for i = 1:length(operCondNew.Outputs)
                obj.Outputs(i).Value(obj.NumConds,:) = single(operCondNew.Outputs(i).Value);
            end
            for i = 1:length(operCondNew.StateDerivs)
                obj.StateDerivs(i).Value(obj.NumConds,:) = single(operCondNew.StateDerivs(i).Value);
            end
            obj.FlightCondition(obj.NumConds) = copy(operCondNew.FlightCondition);
            obj.MassProperties.WeightCode(obj.NumConds) = operCondNew.MassProperties.WeightCode;
            obj.MassProperties.Label(obj.NumConds) = operCondNew.MassProperties.Label;
            for i = 1:length(operCondNew.MassProperties.Parameter)
                obj.MassProperties.Parameter(i).Value(obj.NumConds,:) = ...
                    single(operCondNew.MassProperties.Parameter(i).Value);
            end
            for i = 1:length(operCondNew.LinearModel)
                obj.LinearModel(i).A(:,:,obj.NumConds) = ...
                single(operCondNew.LinearModel(i).A);
                obj.LinearModel(i).B(:,:,obj.NumConds) = ...
                single(operCondNew.LinearModel(i).B);
                obj.LinearModel(i).C(:,:,obj.NumConds) = ...
                single(operCondNew.LinearModel(i).C);
                obj.LinearModel(i).D(:,:,obj.NumConds) = ...
                single(operCondNew.LinearModel(i).D);
            end
            %obj.SuccessfulTrim(obj.NumConds) = operCondNew.SuccessfulTrim;
            %obj.Cost(obj.NumConds) = operCondNew.Cost;
            obj.SelectedforAnalysis(obj.NumConds) = false;
            obj.SelectedforDesign(obj.NumConds) = false;
            obj.Color(obj.NumConds, :) = operCondNew.Color; 
        end

        function setOperCondStruct( obj, operCondStruct )
            for i = 1:length(operCondStruct.States)
                obj.States(i) = lacm.ConditionCtrl;
                obj.States(i).Name = operCondStruct.States(i).Name;
                obj.States(i).Units = operCondStruct.States(i).Units;
                obj.States(i).Fix = operCondStruct.States(i).Fix;
                obj.States(i).BasicMode = operCondStruct.States(i).BasicMode;
                obj.States(i).Constrained = operCondStruct.States(i).Constrained;
            end
            for i = 1:length(operCondStruct.Inputs)
                obj.Inputs(i) = lacm.ConditionCtrl;
                obj.Inputs(i).Name = operCondStruct.Inputs(i).Name;
                obj.Inputs(i).Units = operCondStruct.Inputs(i).Units;
                obj.Inputs(i).Fix = operCondStruct.Inputs(i).Fix;
                obj.Inputs(i).BasicMode = operCondStruct.Inputs(i).BasicMode;
                obj.Inputs(i).Constrained = operCondStruct.Inputs(i).Constrained;
            end
            for i = 1:length(operCondStruct.Outputs)
                obj.Outputs(i) = lacm.ConditionCtrl;
                obj.Outputs(i).Name = operCondStruct.Outputs(i).Name;
                obj.Outputs(i).Units = operCondStruct.Outputs(i).Units;
                obj.Outputs(i).Fix = operCondStruct.Outputs(i).Fix;
                obj.Outputs(i).BasicMode = operCondStruct.Outputs(i).BasicMode;
                obj.Outputs(i).Constrained = operCondStruct.Outputs(i).Constrained;
            end
            for i = 1:length(operCondStruct.StateDerivs)
                obj.StateDerivs(i) = lacm.ConditionCtrl;
                obj.StateDerivs(i).Name = operCondStruct.StateDerivs(i).Name;
                obj.StateDerivs(i).Units = operCondStruct.StateDerivs(i).Units;
                obj.StateDerivs(i).Fix = operCondStruct.StateDerivs(i).Fix;
                obj.StateDerivs(i).BasicMode = operCondStruct.StateDerivs(i).BasicMode;
                obj.StateDerivs(i).Constrained = operCondStruct.StateDerivs(i).Constrained;
            end
            obj.MassProperties = lacm.MassPropertiesCtrl;
            for i = 1:length(operCondStruct.MassProperties.Parameter)
                obj.MassProperties.Parameter(i) = lacm.ConditionCtrl;
                obj.MassProperties.Parameter(i).Name = ...
                    operCondStruct.MassProperties.Parameter(i).Name;
                obj.MassProperties.Parameter(i).Units = ...
                    operCondStruct.MassProperties.Parameter(i).Units;
                obj.MassProperties.Parameter(i).Fix = ...
                    operCondStruct.MassProperties.Parameter(i).Fix;
                obj.MassProperties.Parameter(i).BasicMode = ...
                    operCondStruct.MassProperties.Parameter(i).BasicMode;
                obj.MassProperties.Parameter(i).Constrained = ...
                    operCondStruct.MassProperties.Parameter(i).Constrained;
            end
            for i = 1:length(operCondStruct.LinearModel)
                obj.LinearModel(i) = lacm.LinearModel;
                obj.LinearModel(i).Label = ...
                    operCondStruct.LinearModel(i).Label;
                obj.LinearModel(i).States = ...
                    operCondStruct.LinearModel(i).States;
                obj.LinearModel(i).Inputs = ...
                    operCondStruct.LinearModel(i).Inputs;
                obj.LinearModel(i).Outputs = ...
                    operCondStruct.LinearModel(i).Outputs;
                obj.LinearModel(i).AlgebraicInput = ...
                    operCondStruct.LinearModel(i).AlgebraicInput;
                obj.LinearModel(i).AlgebraicOutput = ...
                    operCondStruct.LinearModel(i).AlgebraicOutput;
                obj.LinearModel(i).InputConstraintsArray = ...
                    operCondStruct.LinearModel(i).InputConstraintsArray;
                obj.LinearModel(i).InputConstraintsInfo = ...
                    operCondStruct.LinearModel(i).InputConstraintsInfo;

                Am = size(operCondStruct.LinearModel(i).A, 1);
                An = size(operCondStruct.LinearModel(i).A, 2);
                Bm = size(operCondStruct.LinearModel(i).B, 1);
                Bn = size(operCondStruct.LinearModel(i).B, 2);
                Cm = size(operCondStruct.LinearModel(i).C, 1);
                Cn = size(operCondStruct.LinearModel(i).C, 2);
                Dm = size(operCondStruct.LinearModel(i).D, 1);
                Dn = size(operCondStruct.LinearModel(i).D, 2);

                obj.LinearModel(i).A = single(zeros(Am, An, 1));
                obj.LinearModel(i).B = single(zeros(Bm, Bn, 1));
                obj.LinearModel(i).C = single(zeros(Cm, Cn, 1));
                obj.LinearModel(i).D = single(zeros(Dm, Dn, 1));
            end
            %obj.SuccessfulTrim = false;
            %obj.Cost = 0;
        end
        
        function [y,names] = getDisplayData(obj,ocIdx,fc1,fc2,ic,wc) % Nathan changed
           % Get the filterd operating conditon names in html
            ...
            if ~isempty(obj.Inputs.get(ic))
                y = {num2str(obj.FlightCondition(ocIdx).(fc1),4),...%{sprintf('%.0f 1 ',obj.FlightCondition.(fc1),4),...%
                    num2str(obj.FlightCondition(ocIdx).(fc2),4),...
                    num2str(obj.Inputs.get(ic).Value(ocIdx),4),... % Nathan changed
                    obj.MassProperties.get(wc, ocIdx)}; % Nathan changed
            else
                y = {num2str(obj.FlightCondition(ocIdx).(fc1),4),...%{sprintf('%.0f 1 ',obj.FlightCondition.(fc1),4),...%
                    num2str(obj.FlightCondition(ocIdx).(fc2),4),...
                    num2str(obj.Outputs.get(ic).Value(ocIdx),4),... % Nathan changed
                    obj.MassProperties.get(wc, ocIdx)}; % Nathan changed
            end
            names = {fc1,fc2,ic,wc};
        end % getDisplayData
        
        function y = getDisplayText(obj,ocIdx,fc1,fc2,ic,wc) % Nathan changed
           % Get the filterd operating conditon names in html
            ...
            if obj.SelectedforDesign(ocIdx)
                y = ['<html><font color="rgb(0,0,255)">',fc1,'=',num2str(obj.FlightCondition(ocIdx).(fc1),4),' ',...
                                            fc2,'=',num2str(obj.FlightCondition(ocIdx).(fc2),4),' ',...
                                            ic,'=',num2str(obj.Inputs.get(ic).Value(ocIdx),4),' ',...
                                            wc,'=',obj.MassProperties.get(wc,ocIdx),'</font></html>'];    
            else
                y = ['<html><font color="rgb(',int2str(obj.Color(ocIdx,1)),',',int2str(obj.Color(ocIdx,2)),',',int2str(obj.Color(ocIdx,3)),')">',fc1,'=',num2str(obj.FlightCondition(ocIdx).(fc1),4),' ',...
                                            fc2,'=',num2str(obj.FlightCondition(ocIdx).(fc2),4),' ',...
                                            ic,'=',num2str(obj.Inputs.get(ic).Value(ocIdx),4),' ',...
                                            wc,'=',obj.MassProperties.get(wc,ocIdx),'</font></html>']; 
            end
        ...
        function y = getSelectedDisplayText(obj,ocIdx,fc1,fc2,ic,wc) % Nathan changed
           % Get the filterd operating conditon names in html
            ...
            if ~isempty(obj.Inputs.get(ic))
                ic_str = num2str(obj.Inputs.get(ic).Value(ocIdx),4);
            else
                ic_str = num2str(obj.Outputs.get(ic).Value(ocIdx),4);
            end
            ...
            if strcmp(wc,'WeightCode') || strcmp(wc,'Label')
                ...
                y = ['<html><font color="rgb(',int2str(obj.Color(ocIdx,1)),',',int2str(obj.Color(ocIdx,2)),',',int2str(obj.Color(ocIdx,3)),')">',fc1,'=',num2str(obj.FlightCondition(ocIdx).(fc1),4),' ',...
                                        fc2,'=',num2str(obj.FlightCondition(ocIdx).(fc2),4),' ',...
                                        ic,'=',num2str(ic_str,4),' ',...
                                        wc,'=',num2str(obj.MassProperties.get(wc,ocIdx)),'</font></html>'];
            end
                
        end % getSelectedDisplayText
        
        function y = getUnformattedDisplayText(obj,ocIdx,fc1,fc2,ic,wc) % Nathan changed
           % Get the filterd operating conditon names in html
            ...
            if ~isempty(obj.Inputs.get(ic))
                icvalstr = num2str(obj.Inputs.get(ic).Value(ocIdx),4);
            else
                icvalstr = num2str(obj.Outputs.get(ic).Value(ocIdx),4);
            end
            
            if strcmp(wc,'WeightCode') || strcmp(wc,'Label') % When is this not the case?
                

                
                y = { num2str(obj.FlightCondition(ocIdx).(fc1),4),...
                      num2str(obj.FlightCondition(ocIdx).(fc2),4),...
                      icvalstr,...
                    obj.MassProperties.get(wc,ocIdx)}; 
            else
                y = { num2str(obj.FlightCondition(ocIdx).(fc1),4),...
                      num2str(obj.FlightCondition(ocIdx).(fc2),4),...
                      icvalstr,...
                      num2str(obj.MassProperties.get(wc,ocIdx))};
            end
                
        end % getUnformattedDisplayText
        
        function y = getDisplayTextCell(obj,ocIdx,fc1,fc2,ic,wc) % Nathan changed
           % Get the filterd operating conditon names in html
            ...
            if obj.SelectedforDesign(ocIdx)
                y = {['<html><font color="rgb(0,0,255)">',num2str(obj.FlightCondition(ocIdx).(fc1),4),'</font></html>'],...
                                            ['<html><font color="rgb(0,0,255)">',num2str(obj.FlightCondition(ocIdx).(fc2),4),'</font></html>'],...
                                            ['<html><font color="rgb(0,0,255)">',num2str(obj.Inputs.get(ic).Value(ocIdx),4),'</font></html>'],...
                                            ['<html><font color="rgb(0,0,255)">',obj.MassProperties.get(wc,ocIdx),'</font></html>']};    
            else
                y = {['<html><font color="rgb(',int2str(obj.Color(ocIdx,1)),',',int2str(obj.Color(ocIdx,2)),',',int2str(obj.Color(ocIdx,3)),')">',num2str(obj.FlightCondition(ocIdx).(fc1),4),'</font></html>'],...
                                            ['<html><font color="rgb(',int2str(obj.Color(ocIdx,1)),',',int2str(obj.Color(ocIdx,2)),',',int2str(obj.Color(ocIdx,3)),')">',num2str(obj.FlightCondition(ocIdx).(fc2),4),'</font></html>'],...
                                            ['<html><font color="rgb(',int2str(obj.Color(ocIdx,1)),',',int2str(obj.Color(ocIdx,2)),',',int2str(obj.Color(ocIdx,3)),')">',num2str(obj.Inputs.get(ic).Value(ocIdx),4),'</font></html>'],...
                                            ['<html><font color="rgb(',int2str(obj.Color(ocIdx,1)),',',int2str(obj.Color(ocIdx,2)),',',int2str(obj.Color(ocIdx,3)),')">',obj.MassProperties.get(wc,ocIdx),'</font></html>']}; 
            end
        ...
        function y = eq(A,B) % Needs to change
        ...
        function y = setdiff(A,B) % Needs to change
            if length(A) == length(B)
                if A.FlightCondition == B.FlightCondition &&  ...
                        A.MassProperties == B.MassProperties
                    y = true;
    ...
    methods ( Access = private )
        ...
        function trimLoop( obj )
            ...
            if ~isempty(objH)
                ...
                warning('Can not set initial condition input port ''alt_IC''.');
            end
            ...
            if ~isempty(objH)
                ...
                warning('Can not set initial condition input port ''V_IC''.');
            end
            ...
            % Check if Altitude of Speed was not fixed
            objV = get(obj.States,'V');
            objA = get(obj.States,'xecef');
            ...
            % Update IC of Altitude using the Flight Condition object
            objH = get(obj.Inputs,'alt_IC');
            if ~isempty(objH)
                objH.Value = obj.FlightCondition.Alt*obj.FlightCondition.SimUnitConversionFactor;
            else
                warning('Could not set Inital condition ''alt_IC''.');
            end
            
            % Update IC of V
            objH = get(obj.Inputs,'V_IC');
            if ~isempty(objH)
                temp = obj.FlightCondition.FltCond4Sim;
                objH.Value = temp(3).Value; % Note temp(1)=xecef, temp(2)=alt, temp(3)=V
            else
                warning('Can not set initial condition ''V_IC''.');
            end
