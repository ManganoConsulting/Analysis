...
jaco = zeros(length(i_d_y),length(i_x_u));
cnt = 0;
...
for i = i_x_u'
  ...
  if del_x_u_type(i) == 0
      ...
      % Waggle one generalized input (both sides)
      x_u_left(i)  = x_u(i) - del_x_u(i);
      x_u_right(i) = x_u(i) + del_x_u(i);
      
  elseif del_x_u_type(i) == 1
      
      % Waggle one generalized input (positive side)
      x_u_right(i) = x_u(i) + 2 *del_x_u(i);
      
  elseif del_x_u_type(i) == -1
      
      % Waggle one generalized input (negative side)
      x_u_left(i) = x_u(i) - 2 * del_x_u(i);
  end
  ...
  % Matlab method
%   y_old = feval (sys, t, x_u(1:n_x), x_u(n_x+1:end), 3);
%   y_old = feval (sys, t, x_u(1:n_x), x_u(n_x+1:end), 3);
%   d_old = feval (sys, t, x_u(1:n_x), x_u(n_x+1:end), 1);
...
%   jaco_column = (d_y_right(i_d_y) - d_y_old(i_d_y))/(del_x_u(i));
  jaco(:,cnt) = jaco_column;
end
